...
seed_inputs={}
for number in (20090103,3012009,1032009):
 for order in ('big','little'):
  for size in (16,32):
   entropy=number.to_bytes(size,order)
   for kind in ('bip32','bip39'):
    rawseed=entropy if kind=='bip32' else Bip39SeedGenerator(Bip39MnemonicGenerator().FromEntropy(entropy)).Generate()
    seed_inputs.setdefault(rawseed,[]).append((number,order,size,kind))
for rawseed,names in seed_inputs.items():
 root=Bip32Slip10Secp256k1.FromSeed(rawseed)
 for path in c.PATHS:
  node=root if path=='m' else root.DerivePath(path)
  pub=node.PublicKey().RawCompressed().ToBytes()
  keys.setdefault(pub,[]).append((names,path))
records=list(keys)
...
result={'status':'match' if found else 'exhausted-no-match','family':'newspaper date as integer YYYYMMDD DDMMYYYY MMDDYYYY, BE/LE zero padded 16/32 bytes, BIP32 seed or BIP39 entropy with empty passphrase, prior 214 paths, ordered compressed-key pairs','unique_keys':len(keys),'ordered_pairs':hashes,'control_hits':controls,'matches':len(found),'elapsed_seconds':time.monotonic()-start,'benchmark_hashes_per_second':rate,'estimated_seconds':est}
if found:
 os.umask(0o077)
 (P/'numeric-date-hit.json').write_text(json.dumps([{'a':keys.get(records[i]),'b':keys.get(records[j])} for i,j in found]))
(P/'numeric-date-result.json').write_text(json.dumps(result,indent=2))
