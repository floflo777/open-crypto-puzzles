...
for size in (16,32):
 ...
 for kind in ('bip32','bip39'):
  ...
  seed_inputs.setdefault(rawseed,[]).append((size,kind))
...
result={'status':'match' if found else 'exhausted-no-match','family':'zero 16/32 bytes as BIP32 seed or BIP39 entropy with empty passphrase, prior 214 paths, ordered compressed-key pairs','unique_keys':len(keys),'ordered_pairs':hashes,'control_hits':controls,'matches':len(found),'elapsed_seconds':time.monotonic()-start,'benchmark_hashes_per_second':rate,'estimated_seconds':est}
if found:
 os.umask(0o077)
 (P/'zero-root-hit.json').write_text(json.dumps([{'a':keys.get(records[i]),'b':keys.get(records[j])} for i,j in found]))
(P/'zero-root-result.json').write_text(json.dumps(result,indent=2))
